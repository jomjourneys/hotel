...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (navToggle) {
    navToggle.addEventListener('click', () => {
      ...
      else { mainNav.style.display = 'flex'; mainNav.style.flexDirection = 'column'; mainNav.style.gap = '10px'; }
    });
  }
  ...
  (function heroSlideshow(){
    ...
    function buildDots(){ if (!dotsWrap) return; dotsWrap.innerHTML = ''; slides.forEach((s,i)=>{ const b=document.createElement('button'); b.className='dot'; b.dataset.index=i; b.setAttribute('aria-label','Slide '+(i+1)); b.addEventListener('click', ()=> goTo(i, true)); dotsWrap.appendChild(b); }); }
    function show(i){ slides.forEach(s => s.classList.remove('active')); slides[i].classList.add('active'); const dots=$$('.slide-dots .dot'); dots.forEach((d,idx)=> d.style.background = idx===i ? 'linear-gradient(90deg,#1e90ff,#4da3ff)' : 'rgba(255,255,255,0.6)'); const heroText = document.querySelector('.hero-text'); if (heroText) { heroText.classList.remove('show'); setTimeout(()=> heroText.classList.add('show'), 80); } }
  ...
  /* Smooth scroll to payment section when clicking Pay / Deposit */
  (function payScroll(){
